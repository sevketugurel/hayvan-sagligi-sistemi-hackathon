...
class _HastalarScreenState extends State<HastalarScreen> {
  ...
  String _searchQuery = '';
  String _selectedFilter = 'Hepsi'; // Seçilen filtre
  ...
  List<Hayvan> get _filteredHayvanlar {
    ...
    List<Hayvan> filtered = _hayvanlar;
    ...
    // Sonra türe göre filtrele
    if (_selectedFilter != 'Hepsi') {
      if (_selectedFilter == 'Diğer') {
        // "Diğer" filtresi için standart türlerin hiçbirine uymayan hayvanları göster
        filtered = filtered.where((hayvan) {
          final tur = hayvan.tur.toLowerCase();
          return tur != 'kedi' && tur != 'köpek' && tur != 'kuş';
        }).toList();
      } else {
        // Diğer filtreler için doğrudan tür adına göre filtrele
        filtered = filtered.where((hayvan) {
          return hayvan.tur.toLowerCase() == _selectedFilter.toLowerCase();
        }).toList();
      }
    }
